...
var zen = (function(){
	...
	*********************************************/
	...
	var ElemArray = public.ElemArray =
	function(){
		this.elems	= [];
	...
	ElemArray.prototype.addAttribute = 
	function( name, value ) {
		if( typeof name !== "string" )
	...
	ElemArray.prototype.removeAttribute = 
	function( name ) {
		if( typeof name !== "string" )
	...
	ElemArray.prototype.push = 
	ElemArray.prototype.push_array = 
	function() {
	...
	function parse_group( abbr, index, nest ) {
		var result			= new ElemArray(); //topmost elements
		var enumerated		= null;
		var elem_stack 		= new ElemArray();
				...
				case '^': { //Level up
					...
					while( abbr[i] == '^' || is_white_space(abbr[i]) ) {
						if( abbr[i] == '^' ) {
							...
							if( last_elem )
								elem_stack.push(last_elem); //get last elem
						}
	...
	function enumerate(elem, index) {
	  ...
	  //
	  //e.g.
	  // swap_non_special_chars( "Vodka contains %\% of alcohol", "%", "40")
	  //will result with
